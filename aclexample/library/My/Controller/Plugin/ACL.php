 		...
 		public function preDispatch(Zend_Controller_Request_Abstract $objRequest) {
	        ...
	        if( $objAuth->hasIdentity() ){
	            ...
	            $arrUser = $objAuth->getIdentity();
                    $arrRoles = Model_RoleTable::getInstance()->findById($arrUser->role_id)->toArray();
	            ...
	            if( ! $objAcl->isAllowed($roleName, $objRequest->getModuleName() .'::' .$objRequest->getControllerName() .'::' .$objRequest->getActionName())) {
	            ...
	            if( ! $objAcl->isAllowed($this->strDefaultRole, $objRequest->getModuleName() .'::' .$objRequest->getControllerName() .'::' .$objRequest->getActionName())) {
	            	...
	            	return Zend_Controller_Action_HelperBroker::getStaticHelper('redirector')->setGotoRoute(array(),"index");	
